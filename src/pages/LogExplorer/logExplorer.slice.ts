...
const logExplorerSlice = createSlice({
  ...
  reducers: {
    ...
    setSearchField(
      state,
      { payload }: PayloadAction<LogFilters["searchField"]>,
    ) {
    ...
    setSort(
      state,
      { payload }: PayloadAction<{ field: SortField; order: SortOrder }>,
    ) {
      state.sort = payload;
    },
    ...
    setSelectedLog(
      state,
      { payload }: PayloadAction<LogExplorerState["selectedLog"]>,
    ) {
  ...
  (allLogs, filters, sort, pagination) => {
    ...
    if (filters.keyword.trim()) {
      ...
      result = result.filter((log) => {
        switch (filters.searchField) {
          ...
          case "source_ip":
            return log.source_ip.includes(kw);
          case "service":
            return log.service.toLowerCase().includes(kw);
          default:
            return (
    ...
    if (filters.dateTo) {
      result = result.filter(
        (log) => log.timestamp <= filters.dateTo + "T23:59:59Z",
      );
    }
