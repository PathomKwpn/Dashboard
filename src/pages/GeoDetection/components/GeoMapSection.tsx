...
const GeoMapSection = ({ data, loading }: Props) => {
  ...
  const blockRate =
    totalAttacks > 0 ? Math.round((totalBlocked / totalAttacks) * 100) : 0;

  return (
    <Card className="border-border/40 shadow-sm gap-0 py-0 h-full flex flex-col">
      ...
      <CardContent className="px-4 pt-0 pb-4 flex-1 min-h-0">
        {loading ? (
          ...
          <GeoMapChart data={data} height="100%" />
        )}
      </CardContent>
    </Card>
  );
